...
NAN_METHOD(mouseClick)
{
	...
	if (info.Length() == 2)
	{
		doubleC = info[1]->BooleanValue(v8::Isolate::GetCurrent());
	}
...
BMP buildBMP(Local<Object> info)
{
	...
	Local<Context> ctx = Nan::GetCurrentContext();
			...
			// v8::Local<v8::Value> v(a->Get(,i).ToLocalChecked());
	img.width = obj->Get(ctx,
		Nan::New("width").ToLocalChecked()
	).ToLocalChecked()->Uint32Value(Nan::GetCurrentContext()).FromJust();

	img.height = obj->Get(ctx,
		Nan::New("height").ToLocalChecked()
	).ToLocalChecked()->Uint32Value(Nan::GetCurrentContext()).FromJust();

	img.byteWidth = obj->Get(ctx,
		Nan::New("byteWidth").ToLocalChecked()
	).ToLocalChecked()->Uint32Value(Nan::GetCurrentContext()).FromJust();

	img.bitsPerPixel = obj->Get(ctx,
		Nan::New("bitsPerPixel").ToLocalChecked()
	).ToLocalChecked()->Uint32Value(Nan::GetCurrentContext()).FromJust();

	img.bytesPerPixel = obj->Get(ctx,
		Nan::New("bytesPerPixel").ToLocalChecked()
	).ToLocalChecked()->Uint32Value(Nan::GetCurrentContext()).FromJust();

	char* buf = node::Buffer::Data(obj->Get(ctx,
		Nan::New("image").ToLocalChecked()
	).ToLocalChecked());
...
// https://github.com/nodejs/node/blob/master/test/addons/worker-addon/binding.cc
